...
parse_seats <- function(input) {
    binary <- gsub("R", 1, gsub("L", 0, gsub("B", 1, gsub("F", 0, input))))
    rows <- strtoi(substr(binary, 1, 7), base = 2)
    cols <- strtoi(substr(binary, 8, 10), base = 2)
    tibble(
        row = rows,
        seat = cols
    )
}
max(parse_seats(input)$row * 8 + parse_seats(input)$seat)

...
library(tidyverse)
input <- read.delim("02_input.txt",
                    header = FALSE,
                    sep = ":",
                    col.names = c("rule", "pw")) 

parse_pw_rules <- function(input) {
    sep_input <- input %>%
        separate(rule, sep = "-| ", into = c("min", "max", "letter")) %>% 
        mutate(pw = gsub(" ", "", pw, fixed = TRUE))
    
    sum(apply(sep_input,
              1,
              function(x) {
                  between(table(unlist(strsplit(x["pw"], split = "")))[x["letter"]],
                          x["min"],
                          x["max"])
              }), na.rm = TRUE)
}
parse_pw_rules(input)
...
parse_pw_rules2 <- function(input) {
    sep_input <- input %>%
        separate(rule, sep = "-| ", into = c("min", "max", "letter")) %>% 
        mutate(pw = gsub(" ", "", pw, fixed = TRUE))
    ...
    sum(apply(sep_input,
              1,
              function(x) {
                  ((substr(x["pw"], x["min"], x["min"]) == x["letter"]) +
                       (substr(x["pw"], x["max"], x["max"]) == x["letter"])) == 1
              }), na.rm = TRUE)
}
parse_pw_rules2(input)

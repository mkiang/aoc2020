...
input <- as.numeric(readLines("01_input.txt"))

product_pair <- function(input, year) {
    x <- input[input %in% (year - input)]
    x[1] * x[2]
}
product_pair(input, 2020)
...
product_trips <- function(input, year) {
    x <- expand.grid(i = input, j = input, k = input)
    u <- unique(unlist(x[x$i + x$j + x$k == year, ]))
    u[1] * u[2] * u[3]
}
product_trips(input, 2020)

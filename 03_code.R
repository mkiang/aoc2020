...
check_slope <- function(input, right, down) {
    trees <- 0
    row <- 1
    col <- 1
    while (row < NROW(input)) {
        row <- row + down
        col <- (col + right) %% nchar(input[1])
        col <- ifelse(col == 0, nchar(input[1]), col)
        
        if (substr(input[row], col, col) == "#") trees <- trees + 1
    }
    trees
}

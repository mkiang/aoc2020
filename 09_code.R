# --- Day 9: Encoding Error --- With your neighbor happily enjoying their video
...
#
# 35 20 15 25 47 40 62 55 65 95 102 117 150 182 127 219 299 277 309 576 In this
# example, after the 5-number preamble, almost every number is the sum of two of
...
input <- as.numeric(readLines("09_input.txt"))

return_first_number <- function(input, preamble_size = 25) {
    i <- 1
    for (j in (preamble_size + 1):NROW(input)) {
        sub_input <- input[i:(i + preamble_size - 1)]
        nums <- sub_input[sub_input %in% (input[j] - sub_input)]
        if (NROW(nums) == 0) {
            return(input[j])
        }
...
return_first_number(input)

# --- Part Two --- The final step in breaking the XMAS encryption relies on the
...
#
# 35 20 15 25 47 40 62 55 65 95 102 117 150 182 127 219 299 277 309 576 In this
# list, adding up all of the numbers from 15 through 40 produces the invalid
# number from step 1, 127. (Of course, the contiguous set of numbers in your
...
return_range_of_set <- function(input, target_value) {
    i <- 1
    j <- 2
    
    holder <- NULL
    while (j <= NROW(input)) {
        slice_sum <- sum(input[i:j])
  
        if (slice_sum == target_value) return(min(input[i:j]) + max(input[i:j]))
        
        if (slice_sum < target_value) j <- j + 1
        
        if (slice_sum > target_value) {
            i <- i + 1
            j <- i + 1
        }
    }
}
return_range_of_set(input, target_value = return_first_number(input))

...
# following program:
#
# nop +0 acc +1 jmp +4 acc +3 jmp -3 acc -99 acc +1 jmp -4 acc +6 These
# instructions are visited in this order:
#
# nop +0  | 1 acc +1  | 2, 8(!) jmp +4  | 3 acc +3  | 6 jmp -3  | 7 acc -99 |
# acc +1  | 4 jmp -4  | 5 acc +6  | First, the nop +0 does nothing. Then, the
# accumulator is increased from 0 to 1 (acc +1) and jmp +4 sets the next
# instruction to the other acc +1 near the bottom. After it increases the
# accumulator from 1 to 2, jmp -4 executes, setting the next instruction to the
# only acc +3. It sets the accumulator to 5, and jmp -3 causes the program to
...
execute_input <- function(input) {
    steps_already_run <- NULL
    accumulator <- 0
    current_step <- 1
    
    repeat {
        if (current_step %in% steps_already_run) return(accumulator)
        steps_already_run <- c(steps_already_run, current_step)
        
        instr <- input[current_step]
        i_num <- as.numeric(substr(instr, 4, nchar(instr)))
        
        if (grepl("acc", instr)) accumulator <- accumulator + i_num
        
        current_step <- ifelse(grepl("jmp", instr),
                               current_step + i_num,
                               current_step + 1)
        
        if (current_step > NROW(input)) {
            names(accumulator) <- current_step
            return(accumulator)
        }
...
# --- Part Two --- After some careful analysis, you believe that exactly one
...
#
# nop +0 acc +1 jmp +4 acc +3 jmp -3 acc -99 acc +1 jmp -4 acc +6 If you change
# the first instruction from nop +0 to jmp +0, it would create a
...
#
# nop +0  | 1 acc +1  | 2 jmp +4  | 3 acc +3  | jmp -3  | acc -99 | acc +1  | 4
# nop -4  | 5 acc +6  | 6 After the last instruction (acc +6), the program
...
# terminates?--- Part Two --- After some careful analysis, you believe that
...
indices <- which(grepl("jmp|nop", input))

for (i in 1:NROW(indices)) {
    new_input <- input
    new_input[indices[i]] <-
        ifelse(
            grepl("jmp", new_input[indices[i]]),
            gsub("jmp", "nop", new_input[indices[i]]),
            gsub("nop", "jmp", new_input[indices[i]])
        )
    x <- execute_input(new_input)
    if (!is.null(names(x))) {
        print(x)
    }
}

## Something is wrong here -- returns two numbers. The last one works. Not sure
## why the first one is returned. 
